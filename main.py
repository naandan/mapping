...
def insert_master(sql_server_conn, postgres_conn, *args, **kwargs):
    ...
    def save_master(res, id):
        ...
        if res['email'] is None:
            res['email'] = f"random-{inn}-email@gmail.com"

        if res['alamat'] is None:
            res['alamat'] = f"random-{inn}-alamat"
        ...
        postgres_cursor.execute(
            ...
            """,
            (id, 'password', None, False, str(res['id']), res['nama_depan'], res['nama_belakang'], res['email'], False, True, datetime.now())
        )
        
        postgres_cursor.execute(
            """
            INSERT INTO master_master
            (id, full_name, gender, address, personal_identity, blood_type, marital_status, profile_photo, created_at, updated_at, location_id, user_id)
            VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)
            """,    
            (str(uuid4()), f"{res['nama_depan']} {res['nama_belakang']}", get_gender(res['id_gender']), get_existing(res['alamat']), cleaned_string(res['no_ktp']), get_blood_type(res['id_golongan_darah']), get_marital_status(res['id_marital_status']), None, get_datetime_type(res['created_date']), datetime.now(), location_id, id)
        )
        postgres_conn.commit()
...
def insert_congregation(sql_server_conn, postgres_conn, *args, **kwargs):
    ...
    def save(res):
        ...
        postgres_cursor.execute(
            ...
            """,
            (congregation_id, res['no_reg'], res['nama_alias'], res['nama_mandarin'], True, None, get_datetime_type(res['created_date']), datetime.now(), masteruser_id)
        )
        if res['tgl_baptis']:
            postgres_cursor.execute(
                """
                ...
                """,
                (str(uuid4()), get_datetime_type(res['tgl_baptis']), 0, get_datetime_type(res['created_date']), datetime.now(), None, masteruser_id)
            )
...
def insert_employee(sql_server_conn, postgres_conn, *args, **kwargs):
    ...
    def save(res, inn):
        ...
        res['no_rekening'] = f"random_{inn}"
        res['jabatan'] = f"random_{inn}"
        if res['nik'] in inserted_nik or res['nik'] is None:
            res['nik'] = f"random-{inn}-nik"
        ...
        if len(inserted_master_id) == 0:
            masteruser_id = postgres_cursor.execute(f"SELECT id FROM master_master WHERE full_name = '{res['nama_depan']} {res['nama_belakang']}'").fetchone()[0]
        else:
            placeholders = ", ".join(["%s"] * len(inserted_master_id))
            masteruser_id = postgres_cursor.execute(f"SELECT id FROM master_master WHERE full_name = '{res['nama_depan']} {res['nama_belakang']}' AND id NOT IN ({placeholders})", inserted_master_id).fetchone()[0]
        postgres_cursor.execute(
            """
            INSERT INTO master_employee
            (id, nik, religion, bpjs_employment, bpjs_health, account_number, start_date, employee_position, is_employee, created_at, updated_at, masteruser_id)
            VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)
            """,
            (str(uuid4()), res['nik'], get_religion(res['id_agama']), get_int_type(res['no_bpjs_ketenagakerjaan']), get_int_type(res['no_bpjs_kesehatan']), res['no_rekening'], res['tgl_masuk'], res['jabatan'], True, get_datetime_type(res['created_date']), datetime.now(), masteruser_id)
        )
        postgres_conn.commit()
        inserted_master_id.append(masteruser_id)
    ...
    for inn, row in res.iterrows():
        save(row, inn)
    # res.apply(lambda x: save(x, inn), axis=1)
# insert_employee()
...
def insert_family(sql_server_conn, postgres_conn, *args, **kwargs):
    ...
    def save(res):
        master = postgres_cursor.execute(f"SELECT id FROM master_master WHERE full_name = '{res['nama_depan']} {res['nama_belakang']}'").fetchone()[0]
